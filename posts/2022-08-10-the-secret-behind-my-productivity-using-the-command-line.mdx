...
tags:
  - terminal
  - productivity
description: As a developer, I use the command line everyday. I believe that getting better at it during the years drastically increased my productivity. Here some things I've picked up that go a long the way to help me get work done and that might help you as well.
---

As a developer, I use the command line everyday. I believe that getting better at it during the years drastically increased my productivity. Here some things I've picked up that go a long the way to help me get work done and that might help you as well.
...
Examples:
- `git diff | pbcopy` to share a quick idea to a colleague at work
...
I can also use `pbpaste`, but I don't find that as useful as `pbcopy` because I'm usually pasting content on Slack, Jira, Github, etc, not the other way around.
...
`git branch` lists all branches, `grep fix` filter them using the term "fix", finally `xargs` deletes all these branches in one go.


#### awk

I find `awk` really helpful for extracting data from more complex outputs. This means I can extract only one column of the previous output to continue working with my pipes.
...
However, another option that caught my eyes as a GPU-accelerated terminal and that implements all these features I meantioned -- and much more! -- is [Kitty](https://sw.kovidgoyal.net/kitty/) 🤩

I'm considering moving away from Alacritty+tmux and go all in into Kitty because it could be a _much easier_ way to allow me to:
...
I turn off all animations I can or simply avoid Mac features that have them. I know they look great, but the way I see, they just take time out from me. They are also not fun for my computer to render, __especially__ when I'm sharing my screen over the internet with my colleagues. 😕

## Bonus #2: Automate moving windows around

This is not terminal-related, but one tool that heavly improves my productivity is [Hammerspoon](https://www.hammerspoon.org/).
...
This is my script ([link](https://github.com/lucasprag/dotfiles/blob/main/hammerspoon/init.lua)) as an example. I have shortcuts to move windows to predefined spots on my screen in addition to resize them to ocupy 50%, 75% or 100% of the space. Multi-monitor support is also available. ✨

## Wrapping up

These some things I've picked up during my life as a developer that really helped me do my work. If you have any more tips, let me know on twitter! I hope that you have found something new to help __you__ boost your productivity as well. 🚀
